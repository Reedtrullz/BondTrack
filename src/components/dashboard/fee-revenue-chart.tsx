...
function FeeRevenueTooltip({ active, payload, label }: TooltipProps) {
  ...
  const totalFormatted = formatRuneAmount(String(Math.round(total * 1e8)));
  const bondRewardsFormatted = formatRuneAmount(String(Math.round(bondRewards * 1e8)));
  const poolRewardsFormatted = formatRuneAmount(String(Math.round(poolRewards * 1e8)));
    ...
    <div className="rounded-lg border border-zinc-700 bg-zinc-900/95 p-3 shadow-lg dark:border-zinc-700 dark:bg-zinc-900/95">
      ...
      <div className="space-y-1 text-sm">
        ...
        <div className="flex items-center justify-between gap-6">
          ...
          <span className="font-semibold text-emerald-400">{bondRewardsFormatted}</span>
        </div>
        <div className="flex items-center justify-between gap-6">
          <span className="text-zinc-400">Pool rewards</span>
          <span className="font-semibold text-sky-400">{poolRewardsFormatted}</span>
        </div>
      </div>
    </div>
  );
