...
export function PersonalFeeAudit({ positions, networkApy }: PersonalFeeAuditProps) {
  ...
  return (
    <div
      ...
      role="region"
      aria-label="Personal fee scenario"
    >
      <div className="flex items-center justify-between mb-8">
        <div>
          <h3 className="text-xs font-bold text-zinc-400 uppercase mb-1">Personal Fee Audit</h3>
          <p className="text-sm text-zinc-500">Monthly fee scenario only; not realized rewards</p>
        </div>
      ...
      <div className="flex flex-col items-center gap-8">
        <div className="w-full text-center">
          <div className="text-[10px] font-bold text-zinc-400 uppercase mb-1">Estimated gross rewards</div>
          <div className="text-3xl font-bold text-zinc-900 dark:text-zinc-100 font-mono">
            {formatRuneFromNumber(audit.grossReward)}
          </div>
          <div className="text-xs text-zinc-500">RUNE / mo scenario</div>
        </div>
        ...
        <div className="w-full text-center">
          <div className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase mb-1">Estimated net after operator fee</div>
          <div className="text-4xl font-bold text-emerald-700 dark:text-emerald-300 font-mono">
            {formatRuneFromNumber(audit.netTakeHome)}
          </div>
          <div className="text-xs text-emerald-600/70">RUNE / mo scenario</div>
        </div>
      </div>

      <div className="mt-6 pt-6 border-t border-zinc-100 dark:border-zinc-800">
        <div className="flex items-center justify-between text-sm">
          <span className="text-zinc-500">Estimated monthly leakage</span>
          <div className="flex items-center gap-2">
